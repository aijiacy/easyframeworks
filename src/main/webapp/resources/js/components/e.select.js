...
+function($){
	...
	var Select = function(element, options){
		...
		this.$value.attr('name',options.selectName);
		this.init();
	};
	Select.DEFAULTS = {
		/**
	...
	Select.DEFAULTS.TEMPLATE = '<button type="button" class="btn btn-default" data-toggle="item">' +
		...
		'</button>' +
		'<input name="" type="hidden" data-toggle="value"/>' +
		'<ul class="dropdown-menu" role="menu"></ul>';
	$.fn.getValue = function(){
		if($(this).data('e.select')){
			return $(this).data('e.select').getValue();
		}
	};
	$.fn.getItem = function(){
		if($(this).data('e.select')){
			return $(this).data('e.select').getItem();
		}
	};
	$.fn.setValue = function(value){
		if($(this).data('e.select')){
			return $(this).data('e.select').selectByValue(value);
		}
	};
	$.fn.setSelectItems = function(contents){
		if($(this).data('e.select')){
			return $(this).data('e.select').setItems(contents);
		}
	};
	...
	$.fn.select = function(option){
		return this.each(function(){
			var $this = $(this);
			var data = $this.data('e.select');
			var options = $.extend({}, Select.DEFAULTS, $this.data(), typeof option == 'object' && option);
			if(!data){
				$this.data('e.select', (data = new Select(this, options)));
			}
