...
//				});
		...
		_loadData: function(){
			...
			if(self.sortName && self.sortOrder){
				params['page.sort'] = self.sortName;
				params['page.sort.dir'] = self.sortOrder;
			}
		...
		_initPageNo: function(){
			...
			if((self.pageNo-1) % self.showPage == 0){
				self.pageNo != 1 && pageHtml.push('<li><a href="javascript:void(0);">...</a></li>');
				for(var i=self.pageNo; i<=self.totalPage && i<(self.pageNo+self.showPage); i++){
					pageHtml.push('<li data-value="'+i+'" data-role="pageNo"><a href="javascript:void(0);">'+i+'</a></li>');
				}
				...
				start != 1 && pageHtml.push('<li><a href="javascript:void(0);">...</a></li>');
         ...
         */
        destory: function(){
           this.$element.off().data('e.grid', null).empty();
        },
		/**
		 * 外部查询
		 * @method search
		...
		,_handleResult:function(self,result){
			...
			self.startRecord.text(result.offset + 1);
	...
	$.fn.getGrid = function(){
		return $(this).data('e.grid');
	};
	Grid.DEFAULTS.TEMPLATE = '<div class="table-responsive"><table class="table table-responsive table-bordered grid"><thead><tr><th><div class="btn-group buttons"></div><div class="search"><div class="btn-group select" data-role="condition"></div><div class="input-group" style="width:180px;"><input type="text" class="input-medium form-control" placeholder="Search" data-role="searchValue"><div class="input-group-btn"><button type="button" class="btn btn-default" data-role="searchBtn"><span class="glyphicon glyphicon-search"></span></button></div></div></div></th></tr></thead><tbody><tr><td><div class="colResizePointer"></div><div class="grid-body"><div class="grid-table-head"><table class="table table-bordered"></table></div><div class="grid-table-body"><table class="table table-responsive table-bordered table-hover table-striped"></table></div></div></td></tr></tbody><tfoot><tr><td><div class="records">每页:<div class="btn-group select" data-role="pageSizeSelect"></div>条,显示:<span data-role="start-record">1</span>-<span data-role="end-record">10</span>, 共<span data-role="total-record">0</span>条记录.</div><div><div class="btn-group pages"><ul class="pagination"></ul></div></div></td></tr></tfoot></table></div>';
	var old = $.fn.grid;
	$.fn.grid = function(option){
		return this.each(function(){
			var $this = $(this);
			var data = $this.data('e.grid');
			var options = $.extend({}, Grid.DEFAULTS, $this.data(),typeof option == 'object' && option);
			//这了为了适应API_Builder的调用，修改一下
//			if(!data){
				$this.data('e.grid',(data = new Grid(this, options)));
//			}
